...
static gint sp_dt_ruler_event(GtkWidget *widget, GdkEvent *event, SPDesktopWidget *dtw, bool horiz)
{
    ...
    gdk_window_get_pointer(SP_GTK_CANVAS_WIDGET(dtw->canvas)->window, &wx, &wy, NULL);
    Geom::Point const event_win(wx, wy);

    gint width, height;
    gdk_window_get_geometry(SP_GTK_CANVAS_WIDGET(dtw->canvas)->window, NULL /*x*/, NULL /*y*/, &width, &height, NULL/*depth*/);
    ...
    case GDK_BUTTON_PRESS:
            if (event->button.button == 1) {
                ...
                Geom::Point const event_w(dtw->canvas->window_to_world(event_win));
...
gint sp_dt_guide_event(SPCanvasItem *item, GdkEvent *event, gpointer data)
{
    ...
    SPDesktop *desktop = static_cast<SPDesktop*>(item->canvas->get_data("SPDesktop"));
    ...
    case GDK_BUTTON_RELEASE:
            if (drag_type != SP_DRAG_NONE && event->button.button == 1) {
                ...
                if (moved) {
                    ...
                    if (item->canvas->world_pt_inside_window(event_w)) {
                        switch (drag_type) {
                            case SP_DRAG_TRANSLATE:
                            {
    ...
    case GDK_ENTER_NOTIFY:
    {
            ...
            if ((event->crossing.state & GDK_SHIFT_MASK) && (drag_type != SP_DRAG_MOVE_ORIGIN)) {
                desktop->setCursor(GDK_EXCHANGE);
            }
    ...
    case GDK_LEAVE_NOTIFY:
            ...
            desktop->setCursor(desktop->event_context->cursor_shape);
