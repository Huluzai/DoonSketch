...
sp_svg_view_widget_init (SPSVGSPViewWidget *vw)
{
	...
	vw->canvas = new SPGtkCanvas();
	gtk_widget_pop_colormap ();
	gtk_widget_pop_visual ();
	style = gtk_style_copy (SP_GTK_CANVAS_WIDGET(vw->canvas)->style);
	style->bg[GTK_STATE_NORMAL] = style->white;
	gtk_widget_set_style (SP_GTK_CANVAS_WIDGET(vw->canvas), style);
	gtk_scrolled_window_add_with_viewport (GTK_SCROLLED_WINDOW (vw->sw), SP_GTK_CANVAS_WIDGET(vw->canvas));
	gtk_widget_show (SP_GTK_CANVAS_WIDGET(vw->canvas));

	/* View */
	parent = sp_canvas_item_new (vw->canvas->root(), SP_TYPE_CANVAS_GROUP, NULL);
...
sp_svg_view_widget_view_resized (SPViewWidget *vw, Inkscape::UI::View::View */*view*/, gdouble width, gdouble height)
{
	...
	if (svgvw->resize) {
		gtk_widget_set_size_request (SP_GTK_CANVAS_WIDGET(svgvw->canvas), (int)width, (int)height);
