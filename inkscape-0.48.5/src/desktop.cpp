...
void
SPDesktop::init (SPNamedView *nv, CanvasBase *aCanvas, Inkscape::UI::View::EditWidgetInterface *widget)
{
    _widget = widget;
    ...
    /* Setup Canvas */
    canvas->set_data("SPDesktop", this);
    
    SPCanvasGroup *root = canvas->root();
...
SPDesktop::point() const
{
    ...
    Geom::Point pw = canvas->window_to_world (p);
    p = w2d(pw);
...
SPDesktop::set_display_area (double x0, double y0, double x1, double y1, double border, bool log)
{
    ...
    /* Scroll */
    canvas->scroll_to (x0 * newscale - border, y1 * -newscale - border, clear);
...
SPDesktop::scroll_world (double dx, double dy, bool is_scrolling)
{
    ...
    canvas->scroll_to(viewbox.min()[Geom::X] - dx, viewbox.min()[Geom::Y] - dy, FALSE, is_scrolling);
...
void SPDesktop::setCursor(SPCursorType cursor)
{
