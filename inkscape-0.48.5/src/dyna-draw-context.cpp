...
{
    ddc->cursor_shape = SP_CALLIGRAPHY_CURSOR;
...
{
    if (/*gdk_event_get_axis (event, GDK_AXIS_PRESSURE, &dc->pressure)*/FALSE/*zhangguangjian*/)
        dc->pressure = CLAMP (dc->pressure, DDC_MIN_PRESSURE, DDC_MAX_PRESSURE);
    else
        dc->pressure = DDC_DEFAULT_PRESSURE;

    if (/*gdk_event_get_axis (event, GDK_AXIS_XTILT, &dc->xtilt)*/FALSE/*zhangguangjian*/)
        dc->xtilt = CLAMP (dc->xtilt, DDC_MIN_TILT, DDC_MAX_TILT);
    else
        dc->xtilt = DDC_DEFAULT_TILT;

    if (/*gdk_event_get_axis (event, GDK_AXIS_YTILT, &dc->ytilt)*/FALSE/*zhangguangjian*/)
