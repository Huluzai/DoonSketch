...
sp_object_layout_any_value_changed(GtkAdjustment *adj, SPWidget *spw)
{
    ...
    if (actionkey != NULL) {
        ...
        sp_desktop_canvas(desktop)->force_full_redraw_after_interruptions(0);
        ...
        sp_desktop_canvas(desktop)->end_forced_full_redraws();
    }
...
void sp_select_toolbox_prep(SPDesktop *desktop, GtkActionGroup* mainActions, GObject* holder)
{
    ...
    g_object_set_data(G_OBJECT(spw), "dtw", SP_GTK_CANVAS_WIDGET(sp_desktop_canvas(desktop)));
