...
{
    gint x,y;
    gdk_window_get_pointer (SP_GTK_CANVAS_WIDGET (canvas)->window, &x, &y, NULL);
...
void SPDesktopWidget::init( SPDesktopWidget *dtw )
{
    ...
    /* Canvas */
    dtw->canvas = new SPGtkCanvas();
#if defined(HAVE_LIBLCMS1) || defined(HAVE_LIBLCMS2)
    ((SPGtkCanvas*)(dtw->canvas))->enable_cms_display_adj = prefs->getBool("/options/displayprofile/enable");
#endif // defined(HAVE_LIBLCMS1) || defined(HAVE_LIBLCMS2)
    GTK_WIDGET_SET_FLAGS (SP_GTK_CANVAS_WIDGET (dtw->canvas), GTK_CAN_FOCUS);
    style = gtk_style_copy (SP_GTK_CANVAS_WIDGET (dtw->canvas)->style);
    style->bg[GTK_STATE_NORMAL] = style->white;
    gtk_widget_set_style (SP_GTK_CANVAS_WIDGET (dtw->canvas), style);
    if ( prefs->getBool("/options/useextinput/value", true) )
      gtk_widget_set_extension_events(SP_GTK_CANVAS_WIDGET (dtw->canvas) , GDK_EXTENSION_EVENTS_ALL); //set extension events for tablets, unless disabled in preferences
    g_signal_connect (G_OBJECT (dtw->canvas), "event", G_CALLBACK (sp_desktop_widget_event), dtw);
    gtk_table_attach (GTK_TABLE (canvas_tbl), SP_GTK_CANVAS_WIDGET(dtw->canvas), 1, 2, 1, 2, (GtkAttachOptions)(GTK_FILL | GTK_EXPAND), (GtkAttachOptions)(GTK_FILL | GTK_EXPAND), 0, 0);
    ...
    if ( fromDisplay ) {
        ...
        bool enabled = false;
        if ( SP_GTK_CANVAS(dtw->canvas)->cms_key ) {
            *(SP_GTK_CANVAS(dtw->canvas)->cms_key) = id;
            enabled = !SP_GTK_CANVAS(dtw->canvas)->cms_key->empty();
        }
        cms_adjust_set_sensitive( dtw, enabled );
    }
#endif // defined(HAVE_LIBLCMS1) || defined(HAVE_LIBLCMS2)
    ...
    gtk_widget_grab_focus (SP_GTK_CANVAS_WIDGET(dtw->canvas));
}
...
void cms_adjust_toggled( GtkWidget */*button*/, gpointer data )
{
    ...
    if ( down != SP_GTK_CANVAS(dtw->canvas)->enable_cms_display_adj ) {
        SP_GTK_CANVAS(dtw->canvas)->enable_cms_display_adj = down;
        dtw->requestCanvasUpdate();
        Inkscape::Preferences *prefs = Inkscape::Preferences::get();
...
SPDesktopWidget::requestCanvasUpdate() {
    gtk_widget_queue_draw (SP_GTK_CANVAS_WIDGET (SP_CANVAS_ITEM (this->desktop->main)->canvas));
}
...
{
    GtkWindow *topw = GTK_WINDOW(gtk_widget_get_toplevel(SP_GTK_CANVAS_WIDGET(dtw->canvas)));
...
sp_desktop_widget_update_hruler (SPDesktopWidget *dtw)
{
     ...
     */
    NR::IRect viewbox = sp_desktop_canvas(dtw->desktop)->getViewboxIntegers();
...
sp_desktop_widget_adjustment_value_changed (GtkAdjustment */*adj*/, SPDesktopWidget *dtw)
{
    ...
    dtw->canvas->scroll_to (dtw->hadj->value, dtw->vadj->value, FALSE);
    sp_desktop_widget_update_rulers (dtw);
