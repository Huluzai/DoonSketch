...
sp_tweak_context_init(SPTweakContext *tc)
{
    ...
    event_context->cursor_shape = SP_PUSH_CURSOR;
...
sp_tweak_update_cursor (SPTweakContext *tc, bool with_shift)
{
   ...
   switch (tc->mode) {
       case TWEAK_MODE_MOVE:
           ...
           event_context->cursor_shape = SP_TWEAK_MOVE_CURSOR;
           break;
       ...
       case TWEAK_MODE_SHRINK_GROW:
           ...
           if (with_shift) {
               event_context->cursor_shape = SP_THICKEN_CURSOR;
           } else {
               event_context->cursor_shape = SP_THIN_CURSOR;
           }
           break;
       case TWEAK_MODE_ATTRACT_REPEL:
           ...
           if (with_shift) {
               event_context->cursor_shape = SP_REPEL_CURSOR;
           } else {
               event_context->cursor_shape = SP_ATTRACT_CURSOR;
           }
           break;
       case TWEAK_MODE_ROUGHEN:
           ...
           event_context->cursor_shape = SP_ROUGHEN_CURSOR;
           break;
       case TWEAK_MODE_COLORPAINT:
           ...
           event_context->cursor_shape = SP_COLOR_CURSOR;
           break;
...
{
    if (/*gdk_event_get_axis (event, GDK_AXIS_PRESSURE, &tc->pressure)*/FALSE/*zhangguangjian*/)
    ...
    switch (event->type) {
        ...
        case GDK_BUTTON_PRESS:
            if (event->button.button == 1 && !event_context->space_panning) {
                ...
                desktop->canvas->force_full_redraw_after_interruptions(3);
                tc->is_drawing = true;
                tc->is_dilating = true;
                tc->has_dilated = false;
