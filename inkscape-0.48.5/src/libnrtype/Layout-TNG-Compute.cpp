...
void Layout::Calculator::_buildPangoItemizationForPara(ParagraphInfo *para) const
{
    ...
    for(input_index = para->first_input_index ; input_index < _flow._input_stream.size() ; input_index++) {
        if (_flow._input_stream[input_index]->Type() == CONTROL_CODE) {
            ...
            attribute_font_description->start_index = (guint)para_text.bytes();
            para_text.append(&*text_source->text_begin.base(), text_source->text_length);     // build the combined text
            attribute_font_description->end_index = (guint)para_text.bytes();
    ...
    if (_flow._input_stream[para->first_input_index]->Type() == TEXT_SOURCE) {
        ...
        if (text_source->style->direction.set) {
            ...
            pango_items_glist = pango_itemize_with_base_dir(_pango_context, pango_direction, para_text.data(), 0, (guint)para_text.bytes(), attributes_list, NULL);
    ...
    if (pango_items_glist == NULL) {  // no direction specified, guess it
        pango_items_glist = pango_itemize(_pango_context, para_text.data(), 0, (guint)para_text.bytes(), attributes_list, NULL);
    ...
    pango_get_log_attrs(para_text.data(), (int)para_text.bytes(), -1, NULL, &*para->char_attributes.begin(), para->char_attributes.size());
