...
font_factory::font_factory(void) :
    ...
    fontServer(pango_cairo_font_map_new()),
#else
...
#ifdef USE_PANGO_WIN32
#elif defined(USE_PANGO_OSX)
    pango_cairo_font_map_set_resolution((PangoCairoFontMap *)fontServer, 72);
...
void font_factory::GetUIFamiliesAndStyles(FamilyToStylesMap *map)
{
    ...
    if (map) {
        ...
        for (int currentFamily=0; currentFamily < numFamilies; currentFamily++) {
            ...
            //g_debug("Process family : %s", pango_font_family_get_name(families[currentFamily]));
            
//            if (strcmp(pango_font_family_get_name(families[currentFamily]), "Sans") == 0) {
//                G_BREAKPOINT();
//            }
            ...
            for (int currentFace=0; currentFace < numFaces; currentFace++) {
                ...
                //g_debug("\tProcess face : %s", pango_font_face_get_face_name(faces[currentFace]));
                
                if (pango_font_face_get_face_name(faces[currentFace]) == NULL) {
                    //g_debug("\t\tno face name");
                    continue;
                }
                ...
                if (faceDescr) {
                    ...
                    if (!familyUIName.empty() && !styleUIName.empty()) {
                        ...
                        //    g_debug("\t\tFont face inserted");
                        } else {
                            pango_font_description_free(faceDescr);
                        }
                    } else {
                        //g_debug("\t\thas not family UI name or style UI name");
                        pango_font_description_free(faceDescr);
                    }
                }
            }
