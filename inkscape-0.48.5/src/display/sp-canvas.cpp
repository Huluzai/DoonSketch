...
static void render_buffer(SPCanvas *canvas, SPCanvasBuf &buf);
...
redraw_if_visible (SPCanvasItem *item)
{
    if (item->flags & SP_CANVAS_ITEM_VISIBLE) {
        ...
        if (x0 !=0 || x1 !=0 || y0 !=0 || y1 !=0) {
            item->canvas->request_redraw ((int)(item->x1), (int)(item->y1), (int)(item->x2 + 1), (int)(item->y2 + 1));
        }
    }
}
 ...
 */
void sp_canvas_item_invoke_update (SPCanvasItem *item, Geom::Matrix const &affine, unsigned int flags)
{
...
//    g_return_val_if_fail (GTK_WIDGET_MAPPED (SP_GTK_CANVAS_WIDGET(item->canvas)), -1);    zhangguangjian
    ...
    /* TODO: move to platform relative code zhangguangjian
    gdk_pointer_grab (SP_GTK_CANVAS_WINDOW (item->canvas), FALSE,
    ...
    g_debug("Current / grabbed items rect [%.0f, %.0f, %.0f, %.0f]", item->x1, item->x2, item->y1, item->y2);
...
//    gdk_pointer_ungrab (etime);   zhangguangjian
}
...
sp_canvas_item_grab_focus (SPCanvasItem *item)
{
    g_return_if_fail (item != NULL);
    g_return_if_fail (SP_IS_CANVAS_ITEM (item));
//    g_return_if_fail (GTK_WIDGET_CAN_FOCUS (SP_GTK_CANVAS_WIDGET (item->canvas)));    // temporaly removed, zhangguangjian
    ...
    if (focused_item) {
        ...
        ev.focus_change.window = NULL;//SP_GTK_CANVAS_WINDOW (item->canvas);    /// temporaly removed, zhangguangjian
        ev.focus_change.send_event = FALSE;
        ev.focus_change.in = FALSE;

        item->canvas->emit_event (&ev);
    }

    item->canvas->focused_item = item;
//    gtk_widget_grab_focus (SP_GTK_CANVAS_WIDGET (item->canvas));  // temporaly removed, zhangguangjian
...
sp_canvas_class_init (SPCanvasClass *klass)
{
    ...
    klass->push_update_request = add_idle;
    klass->did_finish_drawing = render_buffer;
}
...
sp_canvas_realize (GtkWidget *widget)
{
    ...
    canvas->pixmap_gc = gdk_gc_new (SP_GTK_CANVAS_WINDOW (canvas));
}
...
{
    GtkWidget *widget = SP_GTK_CANVAS_WIDGET (canvas);
...
///#define CANVAS_OUTPUT_VIA_CAIRO
        ...
        if (((SPCanvasClass *)GTK_OBJECT_GET_CLASS(canvas))->did_finish_drawing) {
            ((SPCanvasClass *)GTK_OBJECT_GET_CLASS(canvas))->did_finish_drawing(canvas, buf);
        }
    }
...
void render_buffer(SPCanvas *canvas, SPCanvasBuf &buf)
{
    int x0 = buf.rect.x0, x1 = buf.rect.x1, y0 = buf.rect.y0, y1 = buf.rect.y1;
    int sw = buf.buf_rowstride / 4;
...
sp_canvas_request_update (SPCanvas *canvas)
{
    ...
    //    add_idle (canvas);
    ((SPCanvasClass *)GTK_OBJECT_GET_CLASS(canvas))->push_update_request(canvas);
}
...
//    if (!GTK_WIDGET_DRAWABLE (canvas)) return;    //zhangguangjian
