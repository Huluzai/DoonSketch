...
sp_canvas_arena_update (SPCanvasItem *item, Geom::Matrix const &affine, unsigned int flags)
{
    ...
    if (arena->cursor) {
        ...
        if (new_arena != arena->active) {
            GdkEventCrossing ec;
            ec.window = NULL; //SP_GTK_CANVAS_WIDGET (item->canvas)->window;    temporal remove , zhangguangjian
            ec.send_event = TRUE;
...
{
    SP_CANVAS_ITEM (data)->canvas->request_redraw (area->x0, area->y0, area->x1, area->y1);
}
