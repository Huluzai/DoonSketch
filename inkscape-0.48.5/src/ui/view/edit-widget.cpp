...
void
EditWidget::setCursor(SPCursorType type)
{
    GdkCursor *cursor = sp_stock_gdk_cursor(type);
    gdk_window_set_cursor(SP_GTK_CANVAS_WINDOW(sp_desktop_canvas(_desktop)), cursor);
    gdk_flush();
}
...
void    //added . zhangguangjian
EditWidget::_document_replaced(SPDesktop* desktop,SPDocument* doc)
{
    ...
    setTitle(SP_DOCUMENT_NAME (desktop->doc()));
}
...
EditWidget::initEdit (SPDocument *doc)
{
    ...
    //added . zhangguangjian
    _document_replaced(_desktop, doc);
    _desktop->connectDocumentReplaced(sigc::mem_fun(this, &EditWidget::_document_replaced));
}
