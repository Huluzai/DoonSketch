...
unsigned combine_motion_events(CanvasBase *canvas, GdkEventMotion &event, gint mask)
{
    GdkEvent *event_next;
    gint i = 0;
    event.x -= round(canvas->getViewbox().left());
    event.y -= round(canvas->getViewbox().top());
