...
sp_text_context_init(SPTextContext *tc)
{
    ...
    event_context->cursor_shape = SP_TEXT_CURSOR;
...
sp_text_context_setup(SPEventContext *ec)
{
    ...
    tc->imc = NULL;//gtk_im_multicontext_new();
    if (tc->imc) {
        GtkWidget *canvas = SP_GTK_CANVAS_WIDGET(sp_desktop_canvas(desktop));
...
sp_text_context_finish(SPEventContext *ec)
{
    ...
    if (ec->desktop) {
//        sp_signal_disconnect_by_data(SP_GTK_CANVAS_WIDGET(sp_desktop_canvas(ec->desktop)), tc);   //zhangguangjian
    }
    ...
    switch (event->type) {
        ...
        case GDK_MOTION_NOTIFY:
            ...
            if (SP_IS_TEXT(item_ungrouped) || SP_IS_FLOWTEXT(item_ungrouped)) {
                ...
                event_context->cursor_shape = SP_TEXT_INSERT_CURSOR;
    ...
    switch (event->type) {
        ...
        case GDK_KEY_PRESS: {
            ...
            if ((tc->text) || (tc->nascent_object)) {
                ...
                if (!tc->imc) { //zhangguangjian
                    if (event->key.string && isprint(event->key.string[0])) {
                        sptc_commit(NULL, event->key.string, tc);
                        imused = true;
                    }
                } else {
                    imused = gtk_im_context_filter_keypress(tc->imc, (GdkEventKey*) event);
                }
                
                if (tc->unimode/* || !tc->imc*/ || !imused
...
//                    sptc_commit(NULL, event->key.string, tc);
            ...
            if (!tc->unimode && imused/*tc->imc && gtk_im_context_filter_keypress(tc->imc, (GdkEventKey*) event)*/) {
                return TRUE;
            }
            break;
        }
